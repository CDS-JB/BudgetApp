<template>
  <div class="loginForm shadow p-3 mb-5 bg-white rounded">
    <h1>Register</h1>
    <p class="loginP">
      Be prepared to become the new Martin Lewis and reach your savings goals!
    </p>
    <form>
      <div class="form-group">
        ...
        <input
          ...
          v-model="form.Email"
          required
          max:100 placeholder="john@smith.com"
        />
      </div>
      <div class="form-group">
        ...
        <input
          type="text"
          class="form-control"
          v-model="form.FirstNm"
          required
          max:100 placeholder="John"
        />
      </div>
      <div class="form-group">
  ...
  data() {
    return {
      ...
      errorModal: {
        ...
        header: { text: "Error" },
        body: {
          text: "Failed to register due to the following error:",
          error: "Debug Error",
        },
      },
    };
  ...
  methods: {
    submit() {
      if (this.form.Password !== this.form.ConfirmPassword) {
        ...
        this.errorModal.body.error =
          "Password must be at least 8 characters long";
        this.errorModal.display = true;
      } else {
        axios
          .put("/api/register", this.form)
          .then((res) => {
            this.$session.start();
            this.$session.set("FirstNm", this.form.FirstNm);
            this.$session.set(
              "BudgetTargetDate",
              moment(this.form.BudgetTargetDate).format("YYYY-MM-DD")
            );
